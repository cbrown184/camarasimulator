...
- [~] Network Access Devices vwip (`/network-access-devices/vwip`; CAMARA
  ...
  - [x] `GET /network-access-devices` (`network-access-devices:reboot`,
    ...
    **subject** is the sole control plane (DESIGN §7; mirrors Number
    ...
    `x-correlator` echoed.
  - [ ] `GET /network-access-devices/{networkAccessDeviceId}`
    (`getNetworkAccessDevice`) — per-device read (stateful; needs a store) —
    deferred.
  - [ ] Reboot Requests resource lifecycle (`POST/GET/PATCH/DELETE
    /reboot-requests…`, `network-access-devices:reboot`) — stateful, in-memory
    store + scheduled/immediate reboot semantics — deferred.
