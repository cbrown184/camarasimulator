...
- [~] Click to Dial vwip (`/click-to-dial/vwip`; CAMARA ClickToDial `wip`;
  ...
  - [~] `status-changed` CloudEvents on `sink`
    ...
    - [x] create-time event — a `createCall` that supplies a `sink` delivers one
      ...
      `https://` a documented no-op cut; ACCESSTOKEN `sinkCredential` bearer
      applied, PLAIN/REFRESHTOKEN a cut; the credential is derived from the
      create body, so no side-store needed). Documented by the `createCall`
      callback in the vendored spec.
    - [x] terminate-time event — a `terminateCall` of a call created with a
      ...
      `DELETE_REQUESTED`. The sink + derived ACCESSTOKEN bearer are recorded at
      create time in a `store` sink side-store (kept apart from the `Call`, so the
      secret is never echoed) and taken single-use at terminate; `store::remove`
