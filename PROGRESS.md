...
- [~] Click to Dial vwip (`/click-to-dial/vwip`; CAMARA ClickToDial `wip`;
  ...
  - [~] `status-changed` CloudEvents on `sink`
    ...
    - [ ] ongoing lifecycle transitions (`connected`/`disconnected`/`failed`,
      with `reason`/`callDuration`/`recordingResult`) + the `terminateCall`
      termination signal — deferred (no live call engine).
    - [ ] TLS (`https://` sink) delivery (needs a rustls TLS client).
