//! Number Recycling **v0.2** (CAMARA Number Recycling 0.2.0, r2.2).
//!
//! One endpoint:
//! - `POST /number-recycling/v0.2/check` — has the subscriber behind a phone
//!   number changed since a reference date?
//!
//! ## What it does
//!
//! The caller submits a `specifiedDate` (a reference calendar date) and,
//! optionally, a `phoneNumber` (two-legged auth only), and the operator answers
//! `{ "phoneNumberRecycled": true|false }`: `true` when the line's subscriber
//! last changed **after** `specifiedDate` (so any binding recorded on or before
//! that date can no longer be trusted), `false` otherwise.
//!
//! The endpoint is protected: it requires a valid access token
//! ([`crate::auth::verify::Claims`]) carrying the `number-recycling:check` scope.
//!
...
//! token **subject**, so resubmitting it is an error (mirrors Call Forwarding
//! Signal):
//!
...
//! - **Recycling verdict (identifier digits × `specifiedDate`).** Otherwise the
//!   identifier's trailing three digits are read as **days since the subscriber
//!   last changed** (`0`–`999`), so the last change happened *today − digits
//!   days*. The number is reported recycled iff that change is strictly after
//!   `specifiedDate` — i.e. `(today − specifiedDate) > digits`. This makes
//!   `specifiedDate` a genuine second control plane: the *same* number reads
//!   `recycled: false` against a recent reference date and `true` against an old
//!   one. An identifier with no digits is treated as *changed today* (`digits =
//!   0`), so it is recycled against any past `specifiedDate`.
//!
//! `specifiedDate` is validated: a malformed or impossible calendar date →
//! `400 INVALID_ARGUMENT`; a date in the future → `400 OUT_OF_RANGE`.
//!
//! Example: `+123456789050` (subscriber changed 50 days ago) with a
//! `specifiedDate` 100 days ago → `recycled: true`; the same number with a
//! `specifiedDate` 30 days ago → `recycled: false`; `+123456789404` →
//! `404 NOT_FOUND`.
...
/// The OAuth2 scope the `POST /check` endpoint requires (CAMARA Number
/// Recycling 0.2.0).
const CHECK_SCOPE: &str = "number-recycling:check";

/// Routes for Number Recycling v0.2, mounted at their canonical URLs.
pub fn routes() -> Router {
    Router::new().route("/number-recycling/v0.2/check", post(check))
}

/// `POST /check` request body (CAMARA `CreateCheckNumRecycling`): a required
/// reference `specifiedDate` and an optional `phoneNumber` (valid only in
/// two-legged auth). Unknown fields are rejected.
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct CreateCheckNumRecycling {
    #[serde(rename = "phoneNumber")]
    phone_number: Option<String>,
    #[serde(rename = "specifiedDate")]
...
async fn check(claims: Claims, headers: HeaderMap, body: Bytes) -> Response {
    ...
    let req: CreateCheckNumRecycling = match serde_json::from_slice(&body) {
        Ok(req) => req,
        Err(_) => {
            return invalid_argument(
                "Request body is not a valid CreateCheckNumRecycling.",
                &correlator,
            )
        }
    };

    // `specifiedDate` must be a well-formed calendar date, and not in the future.
    let Some(specified_days) = parse_date(&req.specified_date) else {
        return invalid_argument(
            "`specifiedDate` must be a valid RFC 3339 full-date (YYYY-MM-DD).",
            &correlator,
        );
    };
    let today = today_unix_days();
    if specified_days > today {
        return out_of_range("`specifiedDate` must not be in the future.", &correlator);
    }
    ...
    // The subscriber last changed `digits` days ago; the number is recycled iff
    // that change is strictly after the reference date.
    let digits = scenarios::trailing_three_digits(&identifier).unwrap_or(0) as i64;
    let last_change_days = today - digits;
    let recycled = last_change_days > specified_days;

    with_correlator(
        (StatusCode::OK, Json(json!({ "phoneNumberRecycled": recycled }))).into_response(),
        &correlator,
    )
}
...
/// Call Forwarding Signal). See the module docs for the four cases.
fn resolve_identifier(
    req: &CreateCheckNumRecycling,
    claims: &Claims,
...
mod tests {
    ...
    fn e164_validation_follows_the_camara_pattern() {
        ...
        assert!(!is_valid_e164("nr-client")); // client-credentials subject
    }
    ...
    async fn mint_token(scope: &str) -> String {
        mint_token_with_client(scope, "nr-client").await
    }
    ...
    async fn post_check(
        ...
        let mut builder = Request::builder()
            .method("POST")
            .uri("/number-recycling/v0.2/check")
            .header("host", HOST)
    ...
    // --- Recycling verdict: identifier digits × specifiedDate --------------

    #[tokio::test]
    async fn old_reference_date_reports_recycled() {
        // Subscriber changed 12 days ago; a very old reference date is long before
        // that change → recycled.
        let (status, _, body) =
            check_ok(r#"{"phoneNumber":"+123456789012","specifiedDate":"2000-01-01"}"#).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(body["phoneNumberRecycled"], true);
    }

    #[tokio::test]
    async fn todays_reference_date_reports_not_recycled() {
        // The subscriber last changed at most today, never strictly after today.
        let body = format!(
            r#"{{"phoneNumber":"+123456789012","specifiedDate":"{}"}}"#,
            ymd_days_ago(0)
        );
        let (status, _, out) = check_ok(&body).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(out["phoneNumberRecycled"], false);
    }

    #[tokio::test]
    async fn specified_date_is_a_real_second_control_plane() {
        // Same number (changed 50 days ago); the reference date alone flips it.
        let recent = format!(
            r#"{{"phoneNumber":"+123456789050","specifiedDate":"{}"}}"#,
            ymd_days_ago(30) // 30 days ago is after the 50-days-ago change → not recycled
        );
        ...
        let older = format!(
            r#"{{"phoneNumber":"+123456789050","specifiedDate":"{}"}}"#,
            ymd_days_ago(100) // 100 days ago is before the change → recycled
        );
        let (status, _, out) = check_ok(&older).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(out["phoneNumberRecycled"], true);
    }
    ...
    async fn reserved_suffix_selects_a_canonical_camara_error() {
        let (status, _, body) =
            check_ok(r#"{"phoneNumber":"+123456789404","specifiedDate":"2000-01-01"}"#).await;
        assert_eq!(status, StatusCode::NOT_FOUND);
        assert_eq!(body["code"], "NOT_FOUND");

        let (status, _, body) =
            check_ok(r#"{"phoneNumber":"+123456789429","specifiedDate":"2000-01-01"}"#).await;
    ...
    async fn three_legged_keys_off_the_subject() {
        // No phoneNumber; subject is an E.164 line changed 12 days ago; old date.
        let token = mint_token_with_client(CHECK_SCOPE, "+123456789012").await;
        let (status, _, body) =
            post_check(Some(&token), r#"{"specifiedDate":"2000-01-01"}"#, None).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(body["phoneNumberRecycled"], true);
    }
    ...
    async fn resubmitting_the_number_on_a_line_token_is_unnecessary() {
        ...
        let (status, _, body) = post_check(
            Some(&token),
            r#"{"phoneNumber":"+123456789012","specifiedDate":"2000-01-01"}"#,
            None,
        )
        .await;
    ...
    async fn no_number_and_non_line_subject_is_missing_identifier() {
        let (status, _, body) = check_ok(r#"{"specifiedDate":"2000-01-01"}"#).await;
        assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
    ...
    async fn malformed_reference_date_is_invalid_argument() {
        let (status, _, body) =
            check_ok(r#"{"phoneNumber":"+123456789012","specifiedDate":"2026-02-30"}"#).await;
        assert_eq!(status, StatusCode::BAD_REQUEST);
    ...
    async fn invalid_phone_format_is_rejected() {
        let (status, _, body) =
            check_ok(r#"{"phoneNumber":"0123","specifiedDate":"2000-01-01"}"#).await;
        assert_eq!(status, StatusCode::BAD_REQUEST);
        assert_eq!(body["code"], "INVALID_ARGUMENT");
    }
    ...
    async fn unknown_field_is_rejected() {
        let (status, _, body) =
            check_ok(r#"{"phoneNumber":"+123456789012","specifiedDate":"2000-01-01","x":1}"#).await;
        assert_eq!(status, StatusCode::BAD_REQUEST);
        assert_eq!(body["code"], "INVALID_ARGUMENT");
    }
