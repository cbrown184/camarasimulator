...
//!   cached ([`tls_connector`]). Mirrors QoD / Geofencing.
//! - **`sinkCredential` (ACCESSTOKEN / PLAIN) auth.** When an assignment is created
//!   with a `sinkCredential`, its credential is applied to the notification's
//!   `Authorization` header ([`sink_authorization`]): a `credentialType: ACCESSTOKEN`
//!   → `Bearer <accessToken>` (RFC 6750, the same scheme CAMARA uses on the resource
//!   server), and a `credentialType: PLAIN` → `Basic <base64(identifier:secret)>`
//!   (RFC 7617 HTTP Basic — the standard application of a plain identifier/secret
//!   pair). The credential is kept **in memory only** (single node, DESIGN §4) and is
//!   never echoed back in an `AssignmentInfo` (it is a secret). The remaining
//!   `credentialType` (`REFRESHTOKEN`) is accepted for schema fidelity but not applied
//!   (it needs a token-exchange round trip) — a documented cut in the served spec.
...
/// testable (mirrors QoD's / Carrier Billing's `sink_authorization`).
...
/// as the `Authorization` header (from the assignment's `sinkCredential`). Any other
...
mod tests {
    ...
    async fn deliver_tls_posts_a_cloudevent_over_a_verified_tls_session() {
        ...
        let parsed: Value = serde_json::from_str(body).expect("body is JSON");
        assert_eq!(parsed["type"], EVENT_TYPE);
        assert_eq!(parsed["data"]["assignmentId"], "tls-assignment");
    }
}
