...
//!   on `http://` loopback), matching QoS Provisioning / QoD / Carrier Billing.
...
/// is sent as the `Authorization` header (from the booking's `sinkCredential`). A
/// non-`http` sink is a no-op success (see the module docs' documented cut). The
/// response is not read — delivery is best-effort.
