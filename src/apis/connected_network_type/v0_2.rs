//! Connected Network Type **v0.2** (CAMARA Connected Network Type 0.2.0, r1.2).
//!
//! One endpoint:
//! - `POST /connected-network-type/v0.2/retrieve` — which mobile communication
//!   technology (`2G`/`3G`/`4G`/`5G`/`UNKNOWN`) is the device connected to right
//!   now? (operationId `getConnectedNetworkType`).
//!
//! ## What it does
//!
...
//! `{ "connectedNetworkType": …, "lastStatusTime": …, "device"?: … }` — the radio
//! technology the device is attached to, never its location.
//!
//! The endpoint is protected: it requires a valid access token
//! ([`crate::auth::verify::Claims`]) carrying the `connected-network-type:read`
//! scope.
//!
//! ## Identifier resolution (two-legged vs three-legged)
//!
//! Faithful to CAMARA (mirrors Simple Edge Discovery): the `device` in the body
...
//! - **Connected network type** — otherwise the identifier's trailing three
//!   digits index a fixed [`NETWORK_TYPES`] table (`digits % 5`; `…000` / no
//!   digits → the first entry, `5G`), so the reported technology is a genuine
//!   control plane (the same input always returns the same type). The order is
//!   newest-first (`5G`, `4G`, `3G`, `2G`, `UNKNOWN`), so the happy-path default
//!   is a modern `5G` attachment.
//!
//! `lastStatusTime` is the current instant (the simulator always treats status as
//! fresh) — **except** for an `UNKNOWN` attachment, where there is no known
//! status time and the field is `null` (the schema marks it nullable).
//!
...
//!
//! Examples: `+123456789000` → `5G` (index 0); `+123456789002` → `3G`
//! (`2 % 5 == 2`); `+123456789004` → `UNKNOWN` with `lastStatusTime: null`;
...
/// The OAuth2 scope the `POST /retrieve` endpoint requires (CAMARA Connected
/// Network Type 0.2.0).
const RETRIEVE_SCOPE: &str = "connected-network-type:read";

/// The connected-network-type values (CAMARA `ConnectedNetworkType` enum),
/// newest-first. The identifier's trailing three digits pick one
/// (`digits % NETWORK_TYPES.len()`), so the reported technology is deterministic
/// from the device — a genuine second control plane (docs/DESIGN.md §7). Index 0
/// (`…000` / no digits) is the happy-path default, a modern `5G` attachment.
const NETWORK_TYPES: [&str; 5] = ["5G", "4G", "3G", "2G", "UNKNOWN"];

/// The value marking "no known radio attachment" — for which there is no
/// meaningful `lastStatusTime` (returned as `null`).
const UNKNOWN: &str = "UNKNOWN";

/// Routes for Connected Network Type v0.2, mounted at their canonical URLs.
pub fn routes() -> Router {
    Router::new().route("/connected-network-type/v0.2/retrieve", post(retrieve))
}

/// `POST /retrieve` request body (CAMARA `ConnectedNetworkTypeRequest`). `device`
...
/// `POST /connected-network-type/v0.2/retrieve`.
async fn retrieve(claims: Claims, headers: HeaderMap, body: Bytes) -> Response {
        ...
        match serde_json::from_slice(&body) {
            ...
            Err(_) => {
                return invalid_argument(
                    "Request body is not a valid ConnectedNetworkTypeRequest.",
                    &correlator,
                )
            }
        }
    ...
    let network_type = connected_network_type(&resolved.identifier);
    // A modern attachment has a fresh status time; UNKNOWN has none (null).
    let last_status_time = if network_type == UNKNOWN {
        Value::Null
    } else {
        Value::String(rfc3339_utc(now_unix_secs()))
    };

    let mut out = json!({
        "connectedNetworkType": network_type,
...
/// The connected network type for `identifier`, chosen from the fixed
/// [`NETWORK_TYPES`] table by the identifier's trailing three digits
/// (`digits % NETWORK_TYPES.len()`; no digits → the first entry, `5G`).
fn connected_network_type(identifier: &str) -> &'static str {
    let idx =
        scenarios::trailing_three_digits(identifier).unwrap_or(0) as usize % NETWORK_TYPES.len();
    NETWORK_TYPES[idx]
}
...
/// Simple Edge Discovery). See the module docs for the cases.
...
/// no date/time dependency (mirrors `device_identifier::v0_3`).
...
mod tests {
    ...
    const HOST: &str = "cnt.local:8080";

    // --- Pure scenario units ----------------------------------------------

    #[test]
    fn network_type_is_indexed_by_the_trailing_digits() {
        // …000 → 0 → 5G (default happy path).
        assert_eq!(connected_network_type("+123456789000"), "5G");
        // No trailing digits → default (5G).
        assert_eq!(connected_network_type("cnt-client"), "5G");
        // …001 → 1 % 5 == 1 → 4G.
        assert_eq!(connected_network_type("+123456789001"), "4G");
        // …002 → 2 % 5 == 2 → 3G.
        assert_eq!(connected_network_type("+123456789002"), "3G");
        // …003 → 3 % 5 == 3 → 2G.
        assert_eq!(connected_network_type("+123456789003"), "2G");
        // …004 → 4 % 5 == 4 → UNKNOWN.
        assert_eq!(connected_network_type("+123456789004"), "UNKNOWN");
        // …012 → 12 % 5 == 2 → 3G.
        assert_eq!(connected_network_type("+123456789012"), "3G");
    }

    #[test]
    fn every_network_type_is_a_valid_camara_enum_value() {
        for t in NETWORK_TYPES {
            assert!(["2G", "3G", "4G", "5G", "UNKNOWN"].contains(&t), "{t} not in the enum");
        }
    }
    ...
    fn e164_validation_follows_the_camara_pattern() {
        ...
        assert!(!is_valid_e164("cnt-client"));
    }
    ...
    async fn mint_token(scope: &str) -> String {
        mint_token_with_client(scope, "cnt-client").await
    }
    ...
    async fn post_retrieve(
        ...
        let mut builder = Request::builder()
            .method("POST")
            .uri("/connected-network-type/v0.2/retrieve")
            .header("host", HOST)
    ...
    #[tokio::test]
    async fn returns_the_network_type_with_required_fields() {
        // …000 → 5G, with a fresh lastStatusTime and (phone-keyed) device echo.
        let (status, _, body) = call_ok(r#"{"device":{"phoneNumber":"+123456789000"}}"#).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(body["connectedNetworkType"], "5G");
    ...
    #[tokio::test]
    async fn different_tails_select_different_types() {
        // …002 → 3G.
        let (status, _, body) = call_ok(r#"{"device":{"phoneNumber":"+123456789002"}}"#).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(body["connectedNetworkType"], "3G");
        // …003 → 2G.
        let (_, _, body) = call_ok(r#"{"device":{"phoneNumber":"+123456789003"}}"#).await;
        assert_eq!(body["connectedNetworkType"], "2G");
    }

    #[tokio::test]
    async fn unknown_type_has_a_null_last_status_time() {
        // …004 → UNKNOWN → lastStatusTime null (no known status).
        let (status, _, body) = call_ok(r#"{"device":{"phoneNumber":"+123456789004"}}"#).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(body["connectedNetworkType"], "UNKNOWN");
        assert!(body["lastStatusTime"].is_null());
    }

    #[tokio::test]
    async fn non_phone_identifier_omits_the_device_echo() {
        // ipv4 publicAddress ending …002 → 3G, no device echo
    ...
    async fn three_legged_keys_off_the_subject() {
        // No device; subject is an E.164 line …002 → 3G, device echoed.
    ...
    async fn no_device_and_non_line_subject_is_missing_identifier() {
        // Two-legged token (sub = cnt-client) with no device → can't identify.
