...
//! - **`sinkCredential` not applied.** Unlike QoD, this API does not yet apply a
//!   `sinkCredential`, so the callback is sent unauthenticated (a documented cut).
...
/// so the receiver sees EOF; `Connection: close` is advertised). A non-`http`
/// sink is a no-op success (see the module docs' documented cut). The response is
/// not read — delivery is best-effort.
